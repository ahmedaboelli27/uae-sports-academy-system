...
import {
  loadPublicEventsContent,
  publicEventsFallbackContent,
...
export default function EventsPage() {
  const [content, setContent] = useState<PublicEventsContentDto>(
    publicEventsFallbackContent,
  );
  ...
  useEffect(() => {
    ...
    loadPublicEventsContent()
      ...
      .catch(() => {
        if (isMounted) {
          setContent(publicEventsFallbackContent);
        }
      })
      .finally(() => {
        if (isMounted) {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return content.events.filter((event) => {
      const matchesSearch =
        ...
        event.branch.toLowerCase().includes(normalizedSearch) ||
        event.city.toLowerCase().includes(normalizedSearch) ||
        event.program.toLowerCase().includes(normalizedSearch) ||
        event.sportName.toLowerCase().includes(normalizedSearch) ||
        event.coachName.toLowerCase().includes(normalizedSearch) ||
        event.category.toLowerCase().includes(normalizedSearch);

      const matchesBranch =
        branchFilter === 'all' || event.branch === branchFilter;
    ...
    <main className="overflow-hidden">
            ...
            <label className="block">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search events, programs, branches, coaches..."
          ...
          <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h2 className="text-3xl font-black tracking-tight text-brand-blue dark:text-white sm:text-4xl">
                Explore upcoming academy activities
              </h2>

              <p className="mt-3 max-w-2xl text-sm leading-7 text-muted-foreground">
                Events are currently powered by scheduled sessions and are ready
                to migrate later to a dedicated public events table.
              </p>
            </div>
          ...
          {filteredEvents.length > 0 ? (
            ...
            <div className="rounded-[2rem] border border-dashed border-border bg-card p-10 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Try changing the search term, branch, program, or category.
              </p>
            </div>
          )}
        </div>
      ...
      <section className="bg-secondary/40 px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-3">
          <FeatureCard
            icon={CalendarCheck}
            title="Schedule-powered events"
            description="The page uses existing schedule sessions until a dedicated event model is added."
          />

          <FeatureCard
            icon={ShieldCheck}
            title="Family-ready information"
            description="Families can see date, time, branch, coach, capacity, and program before booking."
          />

          <FeatureCard
            icon={Trophy}
            title="Connected to programs"
            description="Each event can point families toward the correct program and trial path."
          />
        </div>
      </section>
...
function EventCard({ event }: { event: PublicEventDto }) {
  return (
    <article className="group relative overflow-hidden rounded-[2rem] border border-border bg-card p-5 shadow-sm transition hover:-translate-y-1 hover:border-brand-yellow hover:shadow-lg">
      ...
      <div className="relative mt-5 grid gap-3">
        <MiniInfo icon={CalendarCheck} label="Date" value={event.date} />
        <MiniInfo icon={Clock3} label="Time" value={event.time} />
        <MiniInfo icon={MapPin} label="Branch" value={`${event.branch}, ${event.city}`} />
        <MiniInfo icon={UserRound} label="Coach" value={event.coachName} />
        <MiniInfo icon={Trophy} label="Program" value={event.program} />
        <MiniInfo icon={UsersRound} label="Capacity" value={`${event.capacity} seats`} />
      </div>
